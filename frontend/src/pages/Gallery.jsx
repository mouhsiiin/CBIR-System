...
function Gallery({ onUseAsQuery, showToast }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Stats Bar */}
      <div className="bg-gradient-to-r from-blue-50 to-indigo-50 p-6 rounded-2xl border border-blue-200 shadow-md">
        <div className="flex items-center justify-between">
          ...
          <ImageIcon className="w-12 h-12 text-blue-600 opacity-50" />
        </div>
      </div>
      ...
      {stats?.class_distribution && Object.keys(stats.class_distribution).length > 0 && (
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-lg">
          <h3 className="font-bold text-slate-800 mb-4 text-lg">Detected Object Classes</h3>
          <div className="flex flex-wrap gap-2">
            {Object.entries(stats.class_distribution).map(([className, count]) => (
              <div key={className} className="px-4 py-2 bg-gradient-to-r from-blue-100 to-indigo-100 text-blue-700 rounded-full text-sm font-semibold shadow-sm hover:shadow-md transition-shadow">
                {className}: {count}
              </div>
            ))}
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
          {images.map((image) => (
            <div
              ...
              className={`relative group cursor-pointer rounded-2xl overflow-hidden border-4 transition-all transform hover:scale-105 ${
            ...
            >
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/30 to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end p-4">
                <div className="text-white w-full">
                  <p className="font-semibold text-sm">ID: {image.image_id}</p>
                  <p className="text-xs opacity-90">{image.width} × {image.height}</p>
                </div>
              </div>

              {/* Selection Checkbox */}
              {selectedImages.includes(image.image_id) && (
                <div className="absolute top-3 right-3 w-8 h-8 bg-blue-600 rounded-full flex items-center justify-center shadow-lg animate-scale-in">
